...
import { getHistoryRemote, clearHistoryRemote, deleteHistoryRemote, type HistoryFilters } from '../services/historyService';
...
const HistoryView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      setLoading(true);
      setErrorKey(null);
      getHistoryRemote(filters)
        .then((rows) => {
          if (!cancelled) setHistory(rows);
        })
        .catch((e: any) => {
          if (!cancelled) setErrorKey('history.loadFailed');
        })
        .finally(() => {
  ...
  const getOutputItems = (item: SavedResult) => {
    ...
    const orderMap: Record<string, string[]> = {
      ...
      slab: ['concrete_m3', 'formwork_m2', 'steel_kg', 'bars_qty'],
    ...
    <div className="p-4">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-slate-300">
          ...
          <p className="mt-4 font-medium">{t('history.loadErrorTitle')}</p>
          <p className="text-xs text-slate-400">{t(errorKey)}</p>
        </div>
      ) : history.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-slate-300">
          <HistoryIcon size={64} strokeWidth={1} />
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <div className="text-lg font-black text-slate-800">
                      {item.result.toFixed(getResultDecimals(item))}
